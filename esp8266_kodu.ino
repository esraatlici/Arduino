...
String agAdi = "PS4";                 //Ağımızın adını buraya yazıyoruz.    
String agSifresi = "13102004";           //Ağımızın şifresini buraya yazıyoruz.
...
void loop() {
  ...
  String veri = "GET https://api.thingspeak.com/update?api_key=BF177CVALFUZF4ZB&field1=0";   //Thingspeak komutu. Key kısmına kendi api keyimizi yazıyoruz.                                   //Göndereceğimiz sıcaklık değişkeni
  veri += "&field1=";
  ...
  delay(2000);
  while(!esp.find(">")){                                          //ESP8266 hazır olduğunda içindeki komutlar çalışıyor.
